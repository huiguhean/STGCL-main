...
parser.add_argument('--rnn_units', type=int, default=64, help='hidden dimension')#隐藏维度
...
parser.add_argument('--horizon', type=int, default=12, help='sequence length')#序列长度

parser.add_argument('--method', type=str, default='node', help='two choices: pure, node')
parser.add_argument('--fn_t', type=int, default=12, help='filter negatives threshold, 12 means 1 hour')#过滤负对的阈值，12表示一小时
parser.add_argument('--em_t', type=float, default=0, help='edge masking threshold')       # 边缘屏蔽阈值
parser.add_argument('--im_t', type=float, default=0.01, help='input masking threshold')   # 输入掩码阈值
parser.add_argument('--ts_t', type=float, default=0, help='temporal shifting threshold')  # 时间转移阈值
parser.add_argument('--ism_t', type=float, default=0, help='input smoothing scale')       # 输入平滑阈值
parser.add_argument('--ism_e', type=int, default=20, help='input smoothing entries, which means how much entries we keep untouch during scaling in the frequency domain') # 输入平滑项
parser.add_argument('--tempe', type=float, default=0.1, help='temperature parameter')    # 温度系数
parser.add_argument('--lam', type=float, default=0.1, help='loss lambda')  # 联合学习整体损失的λ
...
for i in range(1, args.epochs + 1):
    ...
    input_diff = []
    t1 = time.time()
    dataloader['train_loader'].shuffle()#在10176做乱序，x和y一起做，扰乱后也对应对，10176个[24*307*2]做随机乱序
